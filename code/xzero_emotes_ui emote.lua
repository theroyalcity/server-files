...
function playEmote(data)
 ...
 function customAnimation()
     ...
     if dictionaries ~= nil then
        ...
        if clip ~= nil then
            lastCli = clip
            local 1oop = openTextInput("","",1)
            local 1 = not (1oop == nil or 1oop == '0' or 1oop == '')
            TriggerEvent('gc:playAmniationAdv', dictionaries, clip, 1)
    end
  end
end

function playAmninationLoop(data)
    TriggerEvent('gc:playAminationAdv', data.dictionaries, data.clip, true)
end

RegisterNetEvent("gc:playEmote")
AddEventHandler("gc:playEmote", function(emoteName)
    local ped = GetPlayerPed(-1);
    if ped then
        TaskStartScenarioInPlace(ped, emoteName, 0, fales)
        emotePlay = true
    end
  end)
